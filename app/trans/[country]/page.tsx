...
interface CountryTransPageProps {
  params: {
    country: string;
  };
}

export async function generateMetadata({ params }: CountryTransPageProps): Promise<Metadata> {
  ...
  return {
    title: `${countryInfo.name} Trans Sex Cams on CAM4 - Live Shemale Webcam Shows`,
    description: `Browse through trans performers from ${countryInfo.name}. Watch live trans sex cams from ${countryInfo.name} shemales broadcasting 24/7.`,
    alternates: {
      canonical: `https://cam4.xxx/trans/${countrySlug}/`
    }
  };
}

export default function CountryTransPage({ params }: CountryTransPageProps) {
  const countrySlug = params.country.toLowerCase();
  const countryInfo = COUNTRY_MAPPINGS[countrySlug];
  ...
  return (
    <div className="w-full px-0 py-2">
      <div className="max-w-[1800px] mx-auto">
        <PageHeader
          title={`${countryInfo.name} Trans Cams on CAM4`}
          description={`Browse through trans performers from ${countryInfo.name}`}
          className="px-2 mb-2"
        />
